...
def training_activity_all(render = None, continuous = True, trainTime = 4):
    """
    Trains all novelties and saves them to the same model
    """
    for novel_name in NoveltyName:
        env = NoveltyDirector(novel_name).build_env(render, continuous)
        start_time = datetime.now()
        end_time = start_time + timedelta(hours=trainTime)
        while datetime.now() < end_time:
            try:
                continueTrainingModel(novel_name, NoveltyName.ALL_NOVELTY)
                sleep(20)
            except NoModelException:
                trainNewModel(env, NoveltyName.ALL_NOVELTY)
...
if __name__ == "__main__":
    ...
    training_activity_all(trainingRenderMode, trainingContinuous, trainingTime)
