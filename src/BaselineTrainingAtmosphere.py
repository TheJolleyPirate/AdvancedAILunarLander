...
def training_baseline_atmosphere(render = None, continuous = True, trainTime = 1):
    """
    Trains baseline agent on the atmosphere novelty only.
    ...
    novelty = NoveltyName.ATMOSPHERE

    while datetime.now() < end_time:
        env = NoveltyDirector(novelty).build_env(render, continuous)
        try:
            # Train the model for a single episode in the novelty
            continueTrainingModel(novelty, novelty)
            sleep(20)
        except NoModelException:
            # Load model and train the next episode in the novelty
            trainNewModel(env, novelty)
...
if __name__ == "__main__":
    ...
    training_baseline_atmosphere(trainingRenderMode, trainingContinuous, trainingTime)